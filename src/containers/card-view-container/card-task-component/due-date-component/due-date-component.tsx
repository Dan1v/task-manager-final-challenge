'use client';
import { format, isBefore, isToday, isYesterday, parseISO } from 'date-fns';
...
export function DueDateComponent({ dueDate }: DueDateComponentProps) {
  ...
  let backGroundColor = 'bg-[#94979A1A]';
  ...
  return (
    <div className={`px-4 py-1 ${backGroundColor} flex gap-2 rounded-[4px] ${textColor}`}>
      <svg
      ...
      <p className={textColor}>{displayText}</p>
    </div>
  );
}
